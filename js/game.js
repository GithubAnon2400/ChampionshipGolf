class Game {
    ...
    updateWind() {
        ...
        if (this.ui) {
            this.ui.elements.gameDetails.querySelector('#wind-info').textContent = `${this.state.wind.speed} mph`;
        }
    }
    ...
    setupMultiplayer() {
        if (!this.course || !this.course.holes || this.course.holes.length === 0) {
            console.error('Course not ready for multiplayer setup');
            return;
        }
    ...
    onBallAtRest() {
        ...
        if (distanceToHole <= 0.054 || (distanceToHole < 0.1 && ballSpeed < 0.1) || distanceToFlagstick < 0.1) {
            ...
            this.createConfetti(); // New confetti effect for fun
    ...
    animate(time) {
        try {
            ...
            if (this.state.isRunning) {
                ...
                this.updateOtherPlayers();
                this.updateWind(); // Dynamic wind updates
            }
