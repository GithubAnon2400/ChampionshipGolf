class Game {
    ...
    initGame() {
        ...
        const seed = `${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()}`.hashCode();
    ...
    selectCharacter(characterName) {
        if (this.characterManager) {
            ...
            if (!this.state.selectedCharacter?.modelGroup) {
                this.state.selectedCharacter.modelGroup = this.createCharacterModel(characterName);
    ...
    createCharacterModel(characterName) {
        ...
        const material = new THREE.MeshStandardMaterial({
            color: characterName.toLowerCase() === 'ace' ? 0x0000ff : 0x00ff00
        });
    ...
    setupMultiplayer() {
        ...
        // Simulated other players with distinct colors
        for (let i = 0; i < 3; i++) {
            ...
            const otherBall = new THREE.Mesh(
                ...
                new THREE.MeshStandardMaterial({ color: 0xff0000 + i * 0x333333 }) // Distinct colors
            );
    ...
    loadNextHole() {
        this.state.currentHole = 1; // Single hole game
        this.state.holeStrokes = 0;

        this.scene.children = this.scene.children.filter(child =>
            child instanceof THREE.Light || child === this.character || child === this.ball || child === this.trail || child === this.aimingIndicator || child === this.holeCircle || child === this.flagstick
        );
    ...
    positionCamera() {
        ...
        const cameraDistance = Math.min(8 + distanceToHole * 0.02, 20); // Dynamic zoom
    ...
    selectBestClub(distanceToHole, greenSize) {
        const ballPos = this.ball.position.clone();
        const isOnGreen = distanceToHole <= greenSize && ballPos.y < 0.2; // On green and near ground
        if (isOnGreen) {
    ...
    swing(power) {
        ...
        this.state.power = Math.min(power, 100);
        ...
        if (this.state.trickShotActive) {
            force *= 1.5;
            direction.y += 0.5;
            this.ui.showNotification('Trick Shot Activated! +50% Power!', 2000);
            this.createTrickShotEffect();
        }
    ...
    onBallAtRest() {
        ...
        const distanceToHole = this.ball.position.distanceTo(holePos);
        const distanceToFlagstick = this.ball.position.distanceTo(this.flagstick.position);
        ...
        if (distanceToHole <= 0.054 || (distanceToHole < 0.1 && ballSpeed < 0.1) || distanceToFlagstick < 0.1) {
            ...
            this.state.playerScore += scoreDifferential;
            ...
            this.endGame(); // End after one hole
        } else {
            if (this.characterObj?.positionAt) {
