class Game {
    constructor() {
        ...
        this.state = {
            ...
            clouds: [],
            wind: { speed: 0, direction: new THREE.Vector3(0, 0, 0) }
        };
        ...
        try {
            ...
            throw e; // Rethrow to trigger error fallback
        }
    ...
    initGame() {
        console.log('Initializing game with UI:', this.ui);
        this.ui.initialize();
        const today = new Date();
        const seed = Utils.hashCode(`${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()}`);
        this.course.generateCourse(seed);

        // Wait for holes to be populated
        const checkHoles = () => {
            console.log('Checking course.holes:', this.course.holes);
            if (this.course.holes && this.course.holes.length > 0 && this.course.holes[0].length !== undefined) {
                this.ui.showScreen('start');
                this.ui.showLoading('');
                this.updateWind(); // Update wind after UI is ready
            } else {
                console.warn('Course holes not ready, retrying in 100ms');
                setTimeout(checkHoles, 100);
            }
        };
        checkHoles();
    }
    ...
    updateWind() {
        if (!this.ui || !this.ui.elements.gameDetails) {
            console.warn('UI or gameDetails not initialized, skipping wind update');
            return;
        }
        this.state.wind.speed = Utils.randomInt(0, 20);
        const angle = Math.random() * Math.PI * 2;
        this.state.wind.direction.set(Math.cos(angle), 0, Math.sin(angle)).normalize();
        const windInfo = this.ui.elements.gameDetails.querySelector('#wind-info');
        if (windInfo) {
            windInfo.textContent = `${this.state.wind.speed} mph`;
        } else {
            console.error('Wind info element not found');
        }
    }
    ...
    swing(power) {
        ...
        // Apply wind effect
        const windEffect = this.state.wind.direction.clone().multiplyScalar(this.state.wind.speed * 0.01);
        direction.x += windEffect.x;
        direction.z += windEffect.z;
        ...
        if (swingSound) {
            ...
            swingSound.play().catch(e => console.warn('Swing sound playback failed:', e));
        }
    }
    ...
    createConfetti() {
        ...
        confetti.position.copy(this.course.getHolePosition());
        this.scene.add(confetti);
        setTimeout(() => this.scene.remove(confetti), 3000);
    }
    ...
    createSparkleEffect(position) {
        ...
        setTimeout(() => this.scene.remove(sparkles), 1000);
    }
    ...
    onBallAtRest() {
        ...
        // Add sparkle effect if close to hole
        if (distanceToHole < 1 && distanceToHole > 0.054) {
            this.createSparkleEffect(this.ball.position);
            this.ui.showNotification('Nice shot! So close!', 2000);
        }
        ...
        if (distanceToHole <= 0.054 || (distanceToHole < 0.1 && ballSpeed < 0.1) || distanceToFlagstick < 0.1) {
            ...
            this.endGame();
            // Play hole-in sound
            const holeInSound = document.getElementById('hole-in-sound');
            if (holeInSound) {
                holeInSound.currentTime = 0;
                holeInSound.play().catch(e => console.warn('Hole-in sound playback failed:', e));
            }
        } else {
            if (this.characterObj?.positionAt) {
